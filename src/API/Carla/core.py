...
class World(object):
    # autopep8: off
    CONSTANT_VELOCITY_60 = 17.0 # 60km/h
    CONSTANT_VELOCITY_30 = 8.5 # 30km/h
    CONSTANT_VELOCITY_10 = 2.83333 # 10km/h
    MODE_ID_INVALID = -1
    MODE_ID_10 = 10
    MODE_ID_30 = 30
    MODE_ID_60 = 60

    LENGTH_OF_STEERING_CACHE = 40
    # autopep8: on
...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        self.constantVelocityModeID = World.MODE_ID_INVALID

    def parse_events(self, client, world, clock, sync_mode, steeringAngle: float):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    world.camera_manager.next_sensor()
                elif event.key == K_6 and (pygame.key.get_mods() & KMOD_CTRL):
                    # constant velocity mode at 60km/h
                    if world.constant_velocity_enabled:
                        ...
                        world.player.enable_constant_velocity(
                            carla.Vector3D(World.CONSTANT_VELOCITY_60, 0, 0))
                        world.constant_velocity_enabled = True
                        self.constantVelocityModeID = World.MODE_ID_60
                        world.hud.notification(
                            "Enabled Constant Velocity Mode at 60 km/h")
                        print("Enabled Constant Velocity Mode at 60 km/h")
                elif event.key == K_3 and (pygame.key.get_mods() & KMOD_CTRL):
                    # constant velocity mode at 30km/h
                    if world.constant_velocity_enabled:
                        world.player.disable_constant_velocity()
                        world.constant_velocity_enabled = False
                        self.constantVelocityModeID = World.MODE_ID_INVALID
                        ...
                        world.player.enable_constant_velocity(
                            carla.Vector3D(World.CONSTANT_VELOCITY_30, 0, 0))
                        world.constant_velocity_enabled = True
                        self.constantVelocityModeID = World.MODE_ID_30
                        world.hud.notification(
                            "Enabled Constant Velocity Mode at 30 km/h")
                        print("Enabled Constant Velocity Mode at 30 km/h")
                elif event.key == K_1 and (pygame.key.get_mods() & KMOD_CTRL):
                    # constant velocity mode at 10km/h
                    if world.constant_velocity_enabled:
                        world.player.disable_constant_velocity()
                        world.constant_velocity_enabled = False
                        self.constantVelocityModeID = World.MODE_ID_INVALID
                        ...
                        world.player.enable_constant_velocity(
                            carla.Vector3D(World.CONSTANT_VELOCITY_10, 0, 0))
                        world.constant_velocity_enabled = True
                        self.constantVelocityModeID = World.MODE_ID_10
                        world.hud.notification(
                            "Enabled Constant Velocity Mode at 10 km/h")
                        print("Enabled Constant Velocity Mode at 10 km/h")
                elif event.key == K_o:
                    try:
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                self._parse_vehicle_keys(
                    pygame.key.get_pressed(), clock.get_time(), steeringAngle)
    ...
    def _parse_vehicle_keys(self, keys, milliseconds, steeringAngle: float):
